<template>
  <div class="overflow-x-auto rounded-md">
    <table class="min-w-full bg-white  shadow text-sm">
      <thead class="bg-green-600 text-white">
        <tr>
          <th class="px-6 py-3 text-left font-semibold">Username</th>
          <th class="px-6 py-3 text-left font-semibold">Role</th>
          <th class="px-6 py-3 text-left font-semibold">Phone Number</th>
          <th class="px-6 py-3  w-[80px]"></th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(user, index) in users" :key="index" class="border-t ">
          <td class="px-6 py-3">{{ user.username }}</td>
          <td class="px-6 py-3 ">{{ user.role }}</td>
          <td class="px-6 py-3">{{ formatPhoneNumber(user.phoneNumber) }}</td>
          <td v-if="currentUser?.role === UserRole.ADMIN || currentUser?.id === user.id"
            class="px-6 py-3 flex justify-end gap-2">
            <button @click="emit('edit', user.id)"
              class="px-3 py-1 bg-white border border-green-600 text-green-600 rounded hover:bg-green-600 hover:text-white transition">
              Edit
            </button>
            <button v-if="currentUser?.role === UserRole.ADMIN && currentUser.id !== user.id"
              @click="emit('delete', user.id)"
              class="px-3 py-1 text-red-600 border border-red-600 rounded hover:bg-red-600 hover:text-white transition">
              Delete
            </button>
          </td>
        </tr>
        <tr v-if="users.length === 0">
          <td colspan="4" class="text-center py-6 text-gray-400">
            No users found.
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</template>

<script setup lang="ts">

import type { UserResponse } from '@/types/user'
import { formatPhoneNumber } from '@/directives/phoneNumber'
import { useAuthStore } from '@/stores/auth-store';
import { UserRole } from '@/enums/user.enum';

defineProps<{
  users: UserResponse[]
}>()

const { user: currentUser } = useAuthStore()
const emit = defineEmits<{
  edit: [id: number]
  delete: [id: number]
}>()
</script>
